...
and request counts, never money. Where the record had a token figure for a result it is kept or put in
the cost figure's place; where a result was recorded in cost only, the sentence says so rather than
converting it. The cost figures remain in the git history, not here.
...
without the hooks and with them, identical answers on every run. Fable 5.1, three runs: on guard 0.2.0, on 0.2.3
with requests going the wrong way 26 → 29, and, run by hand on Windows rather than in a container, with requests
18 → 21. The paired comparison of those runs was recorded in cost only and is not restated here (tokens-only record;
the per-run token rows are in `AB-TASK.md`). All three sit inside the band two identically configured arms have
already produced here, so the Fable reading is a null, not a saving. Opus 5, four hooks-on runs against four
hooks-off runs, likewise compared in cost only. The best on-run is in an earlier version of this paragraph; the
worst is a guard version that trimmed `sed -n` excerpts of source files and taught the model to read in 80-line
ranges, 91 requests for one audit. That is fixed (an excerpt of one file is now read like a Read), and the run after
the fix still came out over the off arm. On Opus 5, on this task, the hook has not shown a saving that survives
repetition; what moves a session most is how the model chooses to read, and the guard's job is not to push it toward
small ranges. The two models do not even spend alike — cache reads weigh most on Opus 5, cache writes on Fable 5.1
(recorded as shares of cost only; not restated here) — so the same hook is pulling a different lever on each. Two
debugging rounds and a small-feature round on Opus 5 saved nothing attributable: the model bounded its own reads and
let 10–14k tokens of tool results in. The saving is whatever the model would otherwise have let in, and on the runs
so far that is nothing to one unrepeated best run at best and worse than nothing at worst, with no result on either
...
alone, because trimming them taught the model to read in eighty-line chunks and ran one audit to 91
requests — and one MCP result, which is not a shell result. The Read cap fired zero times in the same 285. That
...
**Results under the threshold are most of what is carried.** In one working session they were 57% of all
context carried; in an audit session, 79%. Compressing them is what the rest of this category does, and it is
where the JetBrains benchmark found rtk doing more harm than good, so tokenbrake does not — but the share it
declines to touch is the majority of the carried tokens.
...
**No repeatable saving has been demonstrated, by any version, on any workload.** On the read-heavy audit
the shipping guard's three paired runs were compared in cost only (not restated here — tokens-only
record), and none of them showed a saving that held on repetition. Two debugging rounds and a feature
round were flat. The best run ever recorded is one run of eight on that shape and is not reproducible; the
worst, 91 requests for one audit, came from a guard behaviour since removed.
...
sits inside the noise**. The agent simply reads differently
every time.
...
where the tokens are, since a result is read again on every later request. So the hook reliably shrinks
...
going back for what was cut — which is the mechanism's own overhead, now reported by `report`.
...
**The report tells you this about your own sessions, in tokens.** Since 0.2.5 it prints `Within the
...
that is, what the trims took off this session's carried tokens, what is left untrimmed — and how many
return trips the model made for trimmed content. The benchmark found the saving lives in *carried* tokens
(median 197k carried without the hook against 118k with it) rather than in the size of any one result, and
that the number of trims does not predict it (the paired figures behind that were recorded in cost only;
not restated here). On the session that produced the count above
...
withdrawn. Several projects already parse the same `~/.claude/projects/**/*.jsonl` files for token
usage — [token-dashboard](https://github.com/nateherkai/token-dashboard),
...
`/context` and OpenTelemetry export with per-tool attribution. If you want token usage by session or
by day, use one of those; they do it better and they are not attached to a hook.
What this report has that a survey of those did not turn up is narrower: `carried`, which charges tokens
